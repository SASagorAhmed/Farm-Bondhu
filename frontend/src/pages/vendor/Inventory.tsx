...
export default function Inventory() {
  const { data: products = [], isLoading } = useSellerInventory();
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-muted-foreground mt-1">Live available stock and units sold from orders</p>
      </motion.div>
        ...
        <Card className="shadow-card border-l-4" style={{ borderLeftColor: ICON_COLORS.finance }}>
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-2">
              <AlertTriangle className="h-5 w-5" style={{ color: ICON_COLORS.finance }} />
              <h3 className="font-display font-semibold text-foreground">Stock Alerts</h3>
            </div>
            <div className="space-y-1">
              {products.filter((p) => p.stock <= 10 && p.stock > 0).map((p) => (
                <p key={p.id} className="text-sm text-muted-foreground">
                  <span className="font-medium text-foreground">{p.name}</span>
                  {" — only "}
                  <span className="font-bold" style={{ color: ICON_COLORS.health }}>{p.stock}</span>
                  {" available"}
                  {p.units_sold > 0 && (
                ...
                {[...products].sort((a, b) => a.stock - b.stock).map((p) => {
                  ...
                  return (
                    <TableRow key={p.id}>
                      ...
                      <TableCell className="text-right font-medium text-foreground">{p.stock} {p.unit}</TableCell>
