              ...
              <CardContent className="p-5 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {([{ id: "instant" as const, label: "Instant", icon: Zap, desc: "Connect now" }, { id: "scheduled" as const, label: "Schedule", icon: Calendar, desc: "Book for later" }]).map((t) => (
                    <button key={t.id} onClick={() => setBookingType(t.id)} className="p-4 rounded-xl border-2 text-left transition-all" style={bookingType === t.id ? { borderColor: VB, backgroundColor: `${VB}08` } : { borderColor: "hsl(var(--border))" }}>
          ...
          {step === 2 && (
            <Card className="shadow-card overflow-hidden">
              ...
              <CardContent className="p-5 space-y-4">
                ...
                <Button className="w-full h-12 text-white font-bold" style={{ backgroundColor: VB }} onClick={handleConfirm} disabled={submitting}>{submitting ? <Clock className="h-4 w-4 mr-2 animate-spin" /> : <CreditCard className="h-4 w-4 mr-2" />}Pay ৳{vet.fee} & Confirm</Button>
              </CardContent>
            </Card>
          )}
            ...
            <Card className="shadow-card overflow-hidden">
              ...
              <CardContent className="p-6 text-center space-y-4">
                ...
                <div className="flex gap-3 pt-2">
                  {bookingType === "instant" && bookingId && <Button className="flex-1 text-white" style={{ backgroundColor: VB }} onClick={() => navigate(`/vetbondhu/waiting/${bookingId}`)}>Go to Waiting Room</Button>}
