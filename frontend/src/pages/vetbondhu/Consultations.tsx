...
function resolveJoinDestination(booking: JoinableBooking, currentUserId?: string): string | null {
  ...
  if (status === "pending" || status === "confirmed") {
    return `/vetbondhu/waiting/${booking.id}`;
  }
  if (canRejoinNow(booking, currentUserId)) {
    return `/vetbondhu/room/${booking.id}`;
  }
  return null;
}
...
export default function Consultations() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await withApiTiming("/v1/vetbondhu/bookings/bootstrap", () =>
        fetch(`${API_BASE}/v1/vetbondhu/bookings/bootstrap?view=patient&limit=50&offset=${offset}`, {
          cache: "no-store",
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeConsultationBookings({
      channelKey: `vetbondhu-consultations-live-${user.id}`,
      userId: user.id,
      onEvent: (eventType, row) => {
        ...
        if (row?.id) {
          queryClient.invalidateQueries({ queryKey: queryKeys().vetbondhuConsultationRoom(row.id) });
        }
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between flex-wrap gap-3">
        ...
        <Dialog><DialogTrigger asChild><Button className="text-white" style={{ backgroundColor: VB }}><Plus className="h-4 w-4 mr-1" />Book New</Button></DialogTrigger>
          <DialogContent>
            <DialogHeader>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total" value={consultations.length} icon={<Stethoscope className="h-5 w-5" />} iconColor={VB} index={0} />
        <StatCard title="Scheduled" value={scheduled} icon={<CalendarCheck className="h-5 w-5" />} iconColor={VB} index={1} />
        <StatCard title="Completed" value={completed} icon={<CheckCircle className="h-5 w-5" />} iconColor={VB} index={2} />
        <StatCard title="Total Spent" value={`৳${totalSpent}`} icon={<Stethoscope className="h-5 w-5" />} iconColor={VB} index={3} />
      </div>
