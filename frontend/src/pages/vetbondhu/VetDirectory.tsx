  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3">
        <Button variant="ghost" size="icon" onClick={() => navigate("/vetbondhu")}><ArrowLeft className="h-4 w-4" /></Button>
        <div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Button variant={availOnly ? "default" : "outline"} onClick={() => setAvailOnly(!availOnly)} style={availOnly ? { backgroundColor: VB, color: "white", borderColor: VB } : { borderColor: VB, color: VB }}>Available Now</Button>
      </div>
          ...
          return (
            <motion.div key={vet.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
              <Card className="shadow-card hover:shadow-elevated transition-all duration-300 overflow-hidden group h-full flex flex-col">
                <div className="h-1 shrink-0" style={{ backgroundColor: VB }} />
                <CardContent className="p-5 flex flex-col flex-1 space-y-4">
                <div className="flex items-start gap-3">
                  <div className="h-12 w-12 rounded-full flex items-center justify-center shrink-0" style={{ backgroundColor: `${VB}18` }}><Stethoscope className="h-6 w-6" style={{ color: VB }} /></div>
                ...
                <div className="grid grid-cols-3 gap-2 text-center text-sm">
                  ...
                  <div className="p-2 rounded-lg" style={{ backgroundColor: `${VB}0A` }}><p className="text-xs text-muted-foreground">Experience</p><p className="font-bold text-foreground">{vet.experience}yr</p></div>
                  <div className="p-2 rounded-lg" style={{ backgroundColor: `${VB}0A` }}><p className="text-xs text-muted-foreground">Fee</p><p className="font-bold" style={{ color: VB }}>৳{vet.fee}</p></div>
                </div>
                <div className="flex items-center gap-1 text-xs text-muted-foreground"><MapPin className="h-3 w-3 shrink-0" style={{ color: VB }} /><span className="truncate">{vet.location}</span></div>
